      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
        {TICKET_ESTADOS.map(estado => (
          <StatCard 
            ...
            color={`${ESTADO_COLORES[estado].split(' ')[0]} text-white`} // Use background color from ESTADO_COLORES
            onClick={() => setView('tickets')} // TODO: Add filter by status on click
          />
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
          ...
          {proximasCitas.length > 0 ? (
            ...
            <p className="text-gray-500 dark:text-gray-400">No hay citas programadas próximamente.</p>
          )}
        </div>
